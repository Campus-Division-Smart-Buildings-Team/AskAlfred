...
## Phase 5 update (completed 2026-07-21)

Operational rollout is in place. The outcome metrics are exportable and
alertable, dependency faults can be injected safely outside production, the new
result contract and error presenter sit behind flags with a kill-switch,
outcome-rate drift is comparable against a baseline, and the legacy
boolean-success and tuple-return paths are removed.

Implemented:

- Added `core/service_metrics.py`: a query/service-side Prometheus text
  exporter (`render_service_metrics`/`write_service_metrics`) that renders the
  process-wide telemetry counters and the component readiness registry as
  `askalfred_*` series (readiness published as a 1-valued gauge). It reuses the
  Phase 3 label-safety guarantees, so no exception text, IDs, queries, or paths
  become labels. `Telemetry.samples()` was added so the exporter can render
  structured label pairs without re-parsing the flattened snapshot. This
  completes the Phase 3 → Phase 5 hand-off of wiring the readiness snapshot into
  a health surface (plan section H).
- Added `core/alerts.py` and the generated `ops/askalfred_alerts.yml`
  (regenerated by `scripts/gen_alert_rules.py`). A single declarative rule set
  drives both a Prometheus alerting-rules artifact and an in-process
  `evaluate_alerts` used by the operator panel/tests: critical-inconsistent
  request/ingest outcomes, component unavailable/degraded readiness, elevated
  unavailable+failed request rate (volume-gated), degraded-service events, and
  ACL-metadata drops. A test asserts the checked-in artifact matches the
  generated output so drift fails CI.
- Wired readiness, active alerts, request-outcome rates, and rollout flags into
  the operator diagnostics panel (`ui.ui_components._render_operator_health`),
  so an operator can distinguish data-empty, access-empty, partial, unavailable,
  and failed requests without inspecting logs or stack traces. The panel stays
  gated on the Entra operator role and fails safe.
- Added `core/fault_injection.py`: a production-gated fault-injection harness
  with named seams (`FaultPoint`) for the OpenAI embedding/answer, Pinecone
  index-open/query, Redis, auth-callback, registry-write, queue-drain, and
  FRA-rollback paths. Arming is refused when `ENVIRONMENT=production`, and
  `maybe_fail` is a hard no-op in production even if a seam were left armed;
  `FAULT_INJECTION` env auto-arm is honoured only outside production. Seam calls
  were added at each named site inside the existing typed error handling, so an
  injected fault surfaces as the correct `partial`/`unavailable`/etc. outcome
  rather than a crash. Tests prove the harness, the production gate/no-op, and
  that an injected index-open/Redis fault flows through the typed path.
- Added `config/feature_flags.py`: env-backed, read-live flags
  (`use_query_manager`, `use_structured_presenter`, `is_production`). `main.py`
  routes chat input through `use_query_manager()` and the legacy
  `USE_QUERY_MANAGER` constant is now env-configurable. Failure surfaces render
  through a presenter kill-switch (`ui.error_presenter.safe_present_outcome` /
  `safe_present_query_failure`, and a hardened `render_query_failure`): when the
  structured presenter is disabled or raises, presentation degrades to one
  static, user-safe notice with a correlation reference, so a presenter bug can
  never become a new unhandled-exception surface (delivery-risk kill-switch).
- Added `core/outcome_rates.py` and `tools/compare_outcome_rates.py` to compute
  per-status request shares from a telemetry snapshot and compare a current
  window against a baseline, flagging watched statuses (empty/partial/
  unavailable/failed/degraded) whose share rose beyond tolerance, with a
  minimum-volume guard. The tool exits non-zero on regression so it can gate a
  rollout step.
- Removed the legacy compatibility surfaces now that all callers use the
  structured contract. `QueryResult.success` (property, setter, `success=`
  constructor argument, and the `to_dict` boolean) is gone; a single
  `core.outcomes.is_successful(status)` derives the coarse success signal, and
  `to_dict` exposes a derived `successful` field. The legacy tuple router
  (`search_router.execute`, `normalise_execute_result`, the return-type
  aliases), the `semantic_search`/`planon_search`/`maintenance_search` 4-tuple
  wrappers, `SemanticOutcome.as_legacy_tuple`, and the dead `main.py::safe_execute`
  were removed; only the `*_with_outcome` router path remains. Callers and tests
  were migrated.

Regression coverage:

- `tests/test_phase5_operational_rollout.py`: feature-flag defaults/overrides;
  the presenter kill-switch (disabled flag and presenter-raises paths never
  escape); the metrics exporter (counter + readiness rendering, empty case,
  atomic write); alert firing/quiet/volume-gating and the artifact drift guard;
  the fault-injection harness (arm/fire/budget, production refuse + no-op, env
  auto-arm) and two seam integrations (index-open → typed unavailable, Redis →
  raise); outcome-rate counting/comparison/low-volume skip; and the legacy
  removal (no `success` kwarg/attribute, `is_successful`, absent tuple router).
- Existing Phase 0–4 suites were migrated off the removed compatibility symbols
  (`test_outcomes.py`, `test_error_presenter.py`, `test_query_manager.py`,
  `test_phase2_retrieval_outcomes.py`, `test_access_filter_threading.py`).
- Full suite: 574 passed, 5 platform/capability skips (case-insensitive env
  vars and symlink creation on Windows) with a workspace-local pytest temp
  directory; `ruff check .` passes.

Exit criteria status:

- Operators can distinguish data-empty, access-empty, partial, unavailable, and
  failed requests without inspecting stack traces — met (operator health panel +
  Prometheus exporter + alerts).
- User copy is stable, tested, and approved — met (presenter kill-switch keeps a
  stable static fallback; copy suites remain green).
- Legacy boolean-only success and silent-empty failure paths are removed — met
  (`QueryResult.success` and the legacy tuple router deleted; retrieval already
  returns typed source outcomes from Phase 2).

Operational activities that remain deployment-time rather than in-repo: standing
up the Prometheus/Alertmanager/Grafana stack against `ops/askalfred_alerts.yml`
and the exported metrics; executing the fault-injection runs in a live
non-production environment; and capturing the empty/partial/unavailable baseline
against real traffic for `tools/compare_outcome_rates.py`. This milestone
delivers the code and artifacts those activities consume.
